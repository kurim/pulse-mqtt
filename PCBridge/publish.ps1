...
dotnet publish PCBridge.csproj -p:PublishProfile=$profile

if ($LASTEXITCODE -eq 0) {
    $outDir = switch ($Mode) { "Trim" { "publish" } "NoTrim" { "publish_notrim" } "Fx" { "publish_fxdep" } }
    $size   = [math]::Round((Get-Item "$outDir\PCBridge.exe").Length / 1MB, 1)
    Write-Host ""
    Write-Host "✓ .\$outDir\PCBridge.exe  ($size MB)" -ForegroundColor Green
} else {
